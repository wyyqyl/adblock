require.scopes['updater'] = (function() {
  ...
  var TYPE_MANUAL = 1;
  var downloader = new Downloader(getDownloadables, INITIAL_DELAY, CHECK_INTERVAL);
  ...
  function onDownloadSuccess(downloadable, responseText, errorCallback, redirectCallback) {
    ...
    try {
      ...
      if (Info.app in data) {
        ...
        if ("db_version" in updateInfo && "url" in updateInfo &&
            updateInfo.db_version > Info.db_version) {
  ...
  var checkForUpdates = exports.checkForUpdates = function checkForUpdates(_callback) {
    callback = _callback;
