...
require.scopes['filterClasses'] = (function() {
  ...
  var FilterType = {
    ...
    BLOCKING_FILTER: 1,
    WHITELIST_FILTER: 2,
    ELEMENT_HIDE_FILTER: 3,
    ELEMENT_HIDE_EXCEPTION: 4,
    COMMENT_FILTER: 5
  };
  ...
  function Filter(text) {
    this.text = text;
    this.subscriptions = [];
    this.type = FilterType.InvalidFilter;
  }
  ...
  Filter.prototype = {
    ...
    toJSON: function() {
      ...
      if (this.siteKeys !== undefined) {
        result += ', "siteKeys": ' + JSON.stringify(this.siteKeys);
      }
      return result + '}';
    }
  };
   ...
   * @return {String}
   */
  Filter.normalize = function(/**String*/ text) {
    if (!text)
      return text;
   ...
   * @param {Filter} filter
   */
  Filter.increaseHitCount = function(filter) {
   ...
   */
  Filter.resetHitCounts = function(filters) {
    if (!filters) {
      for (var text in Filter.knownFilters) {
        var filter = Filter.knownFilters[text];
        filter.hitCount = 0;
        filter.lastHit = 0;
      }
    } else {
  ...
  function CommentFilter(text) {
    Filter.call(this, text);
    this.type = FilterType.COMMENT_FILTER;
  }
  ...
  ActiveFilter.prototype = {
     ...
     * @return {Boolean}
     */
    isActiveOnDomain: function(/**String*/ docDomain) {
     ...
     * @return {Boolean}
     */
    isActiveOnlyOnDomain: function(/**String*/ docDomain) {
  ...
  function BlockingFilter(text, regexpSource, contentType, matchCase, domains, thirdParty, collapse) {
    ...
    this.type = FilterType.BLOCKING_FILTER;
  ...
  function WhitelistFilter(text, regexpSource, contentType, matchCase, domains, thirdParty, siteKeys) {
    RegExpFilter.call(this, text, regexpSource, contentType, matchCase, domains, thirdParty);
    this.type = FilterType.WHITELIST_FILTER;
    if (siteKeys != null)
  ...
  ElemHideBase.fromText = function(text, domain, isException, tagName, attrRules, selector) {
    if (!selector) {
      ...
      if (attrRules) {
        ...
        for (var idx = 0; idx < attrRules.length; ++idx) {
          var rule = attrRules[idx];
  ...
  function ElemHideFilter(text, domains, selector) {
    ...
    this.type = FilterType.ELEMENT_HIDE_FILTER;
  }
  ...
  function ElemHideException(text, domains, selector) {
    ElemHideBase.call(this, text, domains, selector);
    this.type = FilterType.ELEMENT_HIDE_EXCEPTION;
  }
