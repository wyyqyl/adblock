require.scopes['subscriptionClasses'] = (function() {
   ...
   */
  Subscription.addSubscription = function(subscription, silent) {
    if (subscription.url in Subscription.knownSubscriptions)
      return;

    Subscription.subscriptions.push(subscription);
    Subscription.knownSubscriptions[subscription.url] = subscription;
   ...
   * @param {Boolean} silent  if true, no listeners will be triggered (to be used when filter list is reloaded)
   */
  Subscription.removeSubscription = function(subscription, silent) {
    for (var idx = 0; idx < Subscription.subscriptions.length; ++idx) {
      if (Subscription.subscriptions[idx].url == subscription.url) {
        removeSubscriptionFilters(subscription);
